...
@given(parsers.parse("a duration of {seconds:d} total seconds"), target_fixture="ctx")
...
def when_format_it(ctx):
    dur = timer.Duration(total_seconds=ctx["seconds"], components={"s": ctx["seconds"]})
...
def then_formatted_string(ctx, display):
    assert ctx["formatted"] == display, f"Got {ctx['formatted']!r}, expected {display!r}"
